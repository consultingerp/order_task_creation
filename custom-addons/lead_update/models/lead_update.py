                ...
                for message in lead_rec.message_ids:
                    if message.message_type == 'email':
                        ...
                        if lead_rec.description:
                            ...
                            state = re.search(r'<state>(.*)</state>', desc,
                                              re.DOTALL)
                            if state and state.group(1):
                                state_id = state_rec.search(
                                    [('name', '=', state.group(1))], limit=1)
                                if state_id:
                                    lead_rec.state_id = state_id.id
                            country = re.search(r'<country>(.*)</country>',
                                                desc, re.DOTALL)
                            if country and country.group(1):
                                country_id = state_rec.search(
                                    [('name', '=', country.group(1))], limit=1)
                                if country_id:
                                    lead_rec.country_id = country_id.id

                            pdf_link = re.search(r'<pdf_link>(.*)</pdf_link>',
                                                 desc, re.DOTALL)
                            if pdf_link and pdf_link.group(1):
                                pdf_str = pdf_link.group(1)
                                
                            # Check for description field
                            bod = re.search(r'<body>(.*)</body>', desc,
                                            re.DOTALL)
