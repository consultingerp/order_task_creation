...
class BrandingPosition(models.Model):
    ...
    product_id = fields.Many2one('product.product', string="SKU",
                                 required=True)
    branding_ids = fields.Many2many(
        'branding.item',
        'branding_position_rel',
        'position_id',
        'brand_item',
        string="Branding Item",
        required=True
    )
...
class BrandingItem(models.Model):
    ...
    no_color = fields.Integer(
        string="Number of Color",
        help="Number of color per item"
    )
    ...
    def create(self, vals):
        ...
        product_vals = {'name': vals.get('name'),
                        'default_code':vals.get('brand_item'),
                        'type':'consu',
                        'list_price': vals.get('selling_price'),
                        'standard_price': vals.get('purchase_price')}
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        sql_query = """
                SELECT brand_item from branding_position_rel
                    where position_id = %s
            """
        # for getting the branding item from the
        if context.get('model') == 'sol' and not context.get(
                'product_id') and not context.get('position_id'):
        ...
        elif context.get('model') == 'sol' and context.get(
                'product_id') and not context.get('position_id'):
...
class PreCost(models.Model):
    ...
    def create(self, vals):
        ...
        product_vals = {'name': precost_name,
                        'default_code':vals.get('pre_cost'),
                        'type':'consu',
                        'list_price': vals.get('selling_price'),
                        'standard_price': vals.get('purchase_price')}
        product_id = Product.create(product_vals)
...
class HandlingGroup(models.Model):
    ...
    def create(self, vals):
        Product = self.env['product.product']
        product_vals = {'name': vals.get('name'),
                        'default_code':vals.get('handle_grp'),
                        'type':'consu',
                        'list_price': vals.get('selling_price'),
                        'standard_price': vals.get('purchase_price')}
        product_id = Product.create(product_vals)
