...
class SaleOrder(models.Model):
    ...
    def _get_default_delivery(self):
        ...
        if self.env.user.lang == 'de_DE' or self.partner_id and \
                self.partner_id.lang == 'de_DE':
            delivery_condition = 'Ca. 3-5 Wochen nach Auftragsklarheit'
        return delivery_condition

    refine_brand_ids = fields.One2many('refine.branding.line', 'sale_id',
                                       string='Refinement Lines', copy=False)
    refine_total = fields.Float(string='Total', store=True, readonly=True,
                                compute='_compute_total', copy=False)
    margin = fields.Monetary(compute='_product_margin',
                             help="It gives profitability by calculating the "
                                  "difference between the Unit Price and the "
                                  "cost.",
                             currency_field='currency_id',
                             digits=dp.get_precision('Product Price'))
    emp_email = fields.Char(string="Employee E-mail",
                            compute="_get_user_email")
    layout_product = fields.Char(compute="set_layout_prodyct",
                                 string="Layout Product")
    sale_line_id = fields.Many2one('sale.order.line',
                                   compute="_set_product_line_id",
                                   string="Main Product Line")
    show_tier_price = fields.Boolean("Show Tier Price", default=True,
                                     help="Check for showing Tier prices on "
                                          "Layout Quotation")
    delivery_condition = fields.Char(compute='get_delivery_condition',
                                     string='Delivery Time', readonly=False,
                                     store=True, default=lambda
            self: self._get_default_delivery())
    ...
    def onchange_partner_id(self):
        ...
        if context.get('lang') == 'de_DE':
            ...
            if self.partner_id and self.partner_id.lang == 'de_DE' and \
                self.user_id.lang == 'de_DE':
                delivery_condition = 'Ca. 3-5 Wochen nach Auftragsklarheit'
            elif self.partner_id and self.partner_id.lang == 'de_DE' and \
                self.user_id.lang != 'de_DE':
                delivery_condition = 'Ca. 3-5 Wochen nach Auftragsklarheit'
            elif self.partner_id and self.partner_id.lang == 'en_US' and \
                self.user_id.lang != 'en_US':
                delivery_condition = 'About 3-5 weeks after order clarity'
    ...
    def action_quotation_send(self):
        self.ensure_one()
        '''  Override to use a modified template that includes a portal 
        signup link '''
        action_dict = super(SaleOrder, self).action_quotation_send()
        try:
            template_id = self.env.ref(
                'refinement_new.email_template_edi_sale_layout').id
    ...
    @api.one
    @api.depends('order_line.branding_id', 'order_line.brand_pos_id',
                 'order_line.product_id')
    def _set_product_line_id(self):
        SaleLine = self.env['sale.order.line']
        if len(self.order_line) and isinstance(self.id, int):
            """ 
            -Need to check the branding id and position id for the getting
             the template product
            -Set the line id based on 2 conditions should not be template 
            product should not be branding """
            if any(line.branding_id for line in self.mapped('order_line')):
                self._cr.execute(
                    "SELECT min(id) from sale_order_line where order_id = %s "
                    "and branding_id is not NULL and "
                    "brand_pos_id is not NULL" % self.id)
            else:
                self._cr.execute(
                    "SELECT min(id) from sale_order_line where order_id = "
                    "%s" % self.id)
    ...
    def find_payment_term_trans(self):
        ...
        translation = ''
        for order in self:
            if self.env.user.lang != 'de_DE' and order.partner_id.lang == \
                    'de_DE':
                translation = Translation.search(
                    [('source', '=', order.payment_term_id.name)],
                    limit=1).value
            elif self.env.user.lang == 'de_DE' and order.partner_id.lang == \
                    'de_DE':
                translation = Translation.search(
                    [('value', '=', order.payment_term_id.name)],
                    limit=1).value
            elif self.env.user.lang == 'en_US' and order.partner_id.lang ==  \
                    'en_US':
                translation = Translation.search(
                    [('source', '=', order.payment_term_id.name)],
                    limit=1).source
    ...
    def find_del_tran(self):
        ...
        del_tran =''
        for order in self:
            if order.partner_id.lang == 'de_DE' and order.user_id.lang != \
                    'de_DE':
                del_tran = Translation.search(
                    [('source', '=', order.incoterm.name),
                     ('lang', '=', 'de_DE')], limit=1).value
            elif order.partner_id.lang == 'de_DE' and order.user_id.lang == \
                    'de_DE':
                del_tran = order.incoterm.name
            elif order.partner_id.lang != 'de_DE' and order.user_id.lang == \
                    'de_DE':
                del_tran = Translation.search(
                    [('value', '=', order.incoterm.name)], limit=1).source
    ...
    def find_translation_footer1(self):
        ...
        for order in self:
            if order.partner_id.lang == 'de_DE':
                footer1 = "Kontaktieren Sie uns, wenn Sie Fragen zum " \
                          " Angebotoder veränderte Projektanforderungen haben."
                footer2 = "Wir beraten Sie gerne. Wir sind telefonisch " \
                          "erreichbar von Mo-Fr. von 8:00 bis 18:00 Uhr."
            else:
                footer1 = 'Please contact us in case you have questions ' \
                          'regarding your quotation or changed project ' \
                          'requirements.'
                footer2 = 'We are happy to consult you. Our office hours' \
                          'are Mo-Fr. 8:00 to 18:00.'
    ...
    def find_translation_footer2(self):
        ...
        for order in self:
            if order.partner_id.lang == 'de_DE':
                footer1 = 'Der Zwischenverkauf ist vorbehalten. Aus ' \
                          'technischen Gründen behalten wir uns die Mehr- ' \
                          'oder Minderlieferung von 10% vor.Bei einer ' \
                          'Minderlieferung erstellen wir Ihnen eine ' \
                          'entsprechende Gutschrift. Eine Nachproduktion ' \
                          'oder Ähnliches ist ausgeschlossen.'
                footer2 = 'Bei einer Mehrlieferung wird Ihnen diese ' \
                          'entsprechend berechnet.'
            else:
                footer1 = 'For technical reasons, we reserve the right to ' \
                          'make a short or excess delivery of up to ' \
                          '10%.Any additional costs incurred by film or ' \
                          'repro work will be charged additionally.'
                footer2 = 'We refund the difference in case of a short ' \
                          'delivery.'
            vals.append(str(footer1))
            vals.append(str(footer2))
        return vals
    ...
    def _get_user_email(self):
        ...
        emp_email = Employee.search([('user_id', '=', self.user_id.id)],
                                    limit=1).work_email
    ...
    def _compute_total(self):
        ...
        for order in self:
            ...
            order.update({
                'refine_total': order.pricelist_id.currency_id.round(
                    amount_total),
            })

    @api.depends('refine_brand_ids.tax_id', 'refine_brand_ids.qty',
                 'refine_brand_ids.unit_price')
    def compute_refine_tax(self):
        ...
        for refine_line in self.refine_brand_ids:
            for line_tax_id in refine_line.tax_id:
                refine_taxes = line_tax_id.compute_all(
                                        refine_line.unit_price,
                                        refine_line.sale_id.currency_id,
                                        refine_line.qty,
                                        product=refine_line.product_id,
                                        partner=refine_line.sale_id.partner_id)
                refine_tax_amount += sum(t.get('amount', 0.0) for t in
                                         refine_taxes.get('taxes', []))
        ...
        if product_id.item_ids and qty>1:
            ...
            for item in product_id.mapped('item_ids'):
                if item.min_quantity == qty:
                    price = frm_cur.with_context(ctx).compute(item.fixed_price,
                                                              to_cur,
                                                              round=False)
                else:
                    nearest_item_qty = self.find_nearest_sale_qty(
                        item_price_range, qty)
                    if item.min_quantity == nearest_item_qty[0]:
                        price = frm_cur.with_context(ctx).compute(
                            item.fixed_price, to_cur, round=False)
        else:
            price = frm_cur.with_context(ctx).compute(product_id.list_price,
                                                      to_cur, round=False)
        return price
        ...
        if product_id.seller_ids and qty>1:
            ...
            for seller in product_id.mapped('seller_ids'):
                if seller.min_qty == qty:
                    cost_price = frm_cur.with_context(ctx).compute(
                        seller.price, to_cur, round=False)
                else:
                    nearest_item_qty = self.find_nearest_pur_qty(
                        item_price_range, qty)
            ...
            cost_price = frm_cur.with_context(ctx).compute(
                product_id.standard_price, to_cur, round=False)
        return cost_price
    ...
    def compute_refinement_lines(self):
        ...
        if len(self.refine_brand_ids):
            raise UserError(_(
                'Please reset the refinement lines to compute the Refinement '
                'Cost.'))
            return False
        count = 0
        for line in self.order_line:
            ...
            count+=1
            if line.brand_pos_id and line.branding_id or line.brand_pos_id \
                    or line.branding_id:
                ...
                if line.product_id.handling_group_id:
                    if line.order_id.pricelist_id and line.order_id.partner_id:
                        ...
                        price_unit = self.env[
                            ...
                            product.price, product.taxes_id,
                            line.tax_id)
                        vendor = \
                            line.product_id.handling_group_id.product_id\
                                .seller_ids
                        ...
                        if not seller:
                            cost_price = \
                                line.product_id.handling_group_id.\
                    ...
                    handle_vals = {
                        'product_id':
                            line.product_id.handling_group_id.product_id.id,
                ...
                if line.branding_id.product_id:
                    if line.order_id.pricelist_id and line.order_id.partner_id:
                        product = \
                        line.branding_id.product_id.with_context(
                        lang=line.order_id.partner_id.lang,
                        partner=line.order_id.partner_id.id,
                        quantity=line.product_uom_qty,
                        date_order=line.order_id.date_order,
                        ...
                        pos_price_unit = self.env[
                            'account.tax']._fix_tax_included_price(
                            product.price,
                            product.taxes_id, line.tax_id)
                        vendor = line.branding_id.product_id.seller_ids
                        pos_vendor_id = vendor and vendor[0].name
                        if not pos_vendor_id:
                            cost_price = \
                                line.branding_id.product_id.standard_price
                        else:
                            seller = \
                                line.branding_id.product_id._select_seller(
                            ...
                            if not seller:
                                ...
                                cost_price = self.env[
                                    'account.tax']._fix_tax_included_price(
                                seller.price,
                                line.branding_id.product_id.supplier_taxes_id,
                                line.tax_id)
                    cost_price = \
                        self.find_cost_price(line.branding_id.product_id,
                                             qty=line.product_uom_qty)
                    pos_vals = {'product_id': line.branding_id.product_id.id,
                                ...
                                'qty': line.product_uom_qty,
                                'product_uom_id':
                                    line.branding_id.product_id.uom_id.id,
                        ...
                        if vendor:
                            ...
                            if seller:
                                cost_price = self.env[
                                    'account.tax']._fix_tax_included_price(
                                    seller.price,
                                    pre_cost.product_id.supplier_taxes_id,
                                    line.tax_id)
                        else:
        ...
        for line in self.mapped('order_line'):
            if line.product_id.attribute_value_ids:
                for attr in line.product_id.mapped('attribute_value_ids'):
                    vals.append({'attribute': attr.attribute_id.name,
                                 'value': attr.name})
        return vals
    ...
    def _get_product_pricelist(self):
        ...
        vals = []
        for line in self.mapped('order_line'):
            for product_price_list_item in line.product_id.mapped(
                    'item_ids').sorted(key=lambda b: b.min_quantity):
                vals.append(
                    {'item_name': product_price_list_item.min_quantity,
                     'price': product_price_list_item.fixed_price})
        return vals

    @api.depends('order_line.margin', 'refine_brand_ids.margin')
    def _product_margin(self):
        """Calculating margin on sales order including the marging on
...
class RefineBrandLine(models.Model):
    ...
    def _product_margin(self):
        ...
        for line in self:
            if line.product_id:
                ...
                line_margin = line.sale_id.pricelist_id.currency_id.round(
                    tmp_margin)
            line.margin = tmp_margin

    product_id = fields.Many2one('product.product', string='Product')
    position_id = fields.Many2one('branding.position',
    ...
    price_tax = fields.Monetary(compute='_compute_tax_amount',
                                string='Taxes Amount', readonly=True,
                                store=True)
    sale_line_id = fields.Many2one('sale.order.line', string="Sale Order Line")
    line_id = fields.Integer(string="Sale Line ID")
    currency_id = fields.Many2one(related='sale_line_id.order_id.currency_id',
                                  store=True, string='Currency',
                                  readonly=True)
    margin = fields.Monetary(compute='_product_margin',
                             help="It gives profitability by calculating the "
                                  "difference between the Unit Price and "
                                  "the cost.",
                             currency_field='currency_id',
                             digits=dp.get_precision('Product Price'),
                             store=True)
    ...
    def refinement_calculator(self):
        ...
        for line in self:
            if line.product_id.factor == 'main':
                ...
                """ Fix the code by Finding the main refinery product and 
                use the number of color multiply the num of color into the 
    ...
    def _compute_tax_amount(self):
        for line in self:
            ...
            taxes = line.tax_id.compute_all(price, line.sale_id.currency_id,
                                            line.qty, product=line.product_id,
                                            partner=line.sale_id.partner_id)
            line.update({
...
class SaleOrderLine(models.Model):
    ...
    brand_pos_id = fields.Many2one('branding.position',
                                   string="Branding Position")
    branding_id = fields.Many2one('branding.item', string="Branding Item")
    ...
    def product_id_change(self):
        ...
        if self.product_id.dimension and len(
                self.product_id.dimension.strip()):
            desc += "\n" + "Dimension: " + self.product_id.dimension

        if self.product_id.cust_terif_num and len(
                self.product_id.cust_terif_num.strip()):
            desc += "\n" + "Customs Tariffs Number: " + \
                    self.product_id.cust_terif_num

        if self.product_id.mfg_country:
            desc += "\n" + "Country of Manufacturer: " + \
                    self.product_id.mfg_country.name

        if self.product_id.self_life and len(
                self.product_id.self_life.strip()):
            desc += "\n" + "Shelf Life: " + self.product_id.self_life

        if self.product_id.brand_inf and len(
                self.product_id.brand_inf.strip()):
            desc += "\n" + "Branding Information: " + self.product_id.brand_inf

        if self.product_id.weight and self.product_id.weight > 0.0 or \
                self.product_id.weight < 0.0:
            desc += "\n" + "Weight: " + str(self.product_id.weight)

        if self.product_id.volume and self.product_id.volume > 0.0 or \
                self.product_id.volume < 0.0:
    ...
    def product_id_change_margin(self):
        if not self.order_id.pricelist_id or not self.product_id or not \
                self.product_uom:
            return
        vendor_id = False
    ...
    def write(self, vals):
        for line in self:
            product_uom_qty = vals[
                'product_uom_qty'] if 'product_uom_qty' in vals else False
            if product_uom_qty >= 1 and 'purchase_price' in vals:
                product_id = vals[
                    'product_id'] if 'product_id' in vals else line.product_id
                if isinstance(product_id, (int, long)):
                    product_id = self.env['product.product'].browse(product_id)
                qty = product_uom_qty
                vendor_id = product_id.seller_ids[
                    0].name if product_id.seller_ids else False
