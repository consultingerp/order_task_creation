...
class sale_order(models.Model):
    ...
    task_count = fields.Integer(string='# of Task', compute='_get_task', readonly=True)
    ...
    def action_view_task_source(self):
        ...
        list_view_id = imd.xmlid_to_res_id('order_task_creation.view_task_tree_new')
        form_view_id = imd.xmlid_to_res_id('order_task_creation.view_task_form_sale')
    ...
    def create_task(self):
        ...
        if context.get('lang') == 'de_DE':
            stage_id = self.env['project.task.type'].search([('name', '=', 'Neu')])
        else:
            stage_id = self.env['project.task.type'].search([('name', '=', 'New')])

        if not self.order_line or len(self.order_line)<=0:
            raise UserError(_('There is nothing to create a Sales Task! Please create some order line before creating Sales Task'))
        ...
        return {
                ...
                'view_id': self.env.ref('order_task_creation.view_task_form_sale').id,
...
class project_task(models.Model):
    ...
    refinement_data=fields.Selection([('yes','Yes'),('no','No')],'Refinement Data', index=True)
    delivery_billing_adress=fields.Selection([('yes','Yes'),('no','No')],'Delivery & Billing address', index=True)
    vallue_tax_id=fields.Selection([('yes','Yes'),('no','No')],'Vallue Added Tax ID', index=True)
    delivery_date=fields.Date('Delivery Date', index=True)
    year_week= fields.Char(compute='_compute_yearly_week', index=True)
    payment=fields.Many2many('miscellaneous.entry' ,'payment_manager_task_rel','partner_id' , 'task_id', string='Payment' )
    payment_supplier=fields.Many2many('misc.supplier.payment','supplier_manager_task_rel' , 'supplier_id' , 'task_id1', string='Payment Supplier' )
    digital_sample=fields.Many2many('digital.sample' ,'digital_manager_task_rel', 'digital_id' , 'task_id2', string='Digital Sample' )
    sample=fields.Many2many('sample.sample' ,'sample_manager_task_rel', 'sample_id' , 'task_id3', string='Sample' )
    ...
    #                             domain="['|',('sales_stage','=', True),('project_ids', '=', project_id)]", copy=False)
    ...
    def _read_group_stage_ids_inherit(self, domain, read_group_order=None,
        ...
        if not context.get('default_sale_task'):
            if 'default_project_id' in context:
                search_domain = ['|', ('project_ids', '=', context['default_project_id']), ('id', 'in', self.ids)]
            else:
